...
const MapFitBounds = ({ bounds, padding = [30, 30] }) => {
  const map = useMap();
    ...
    if (bounds && bounds.length > 0 && !hasInitialized.current) {
      try {
        ...
        if (leafletBounds.isValid()) {
          map.fitBounds(leafletBounds, { padding, maxZoom: 10 });
        }
...
const MapZoneEditor = ({ zoneConfig, setZoneConfig, missionType }) => {
  ...
  const config = zoneConfig || getDefaultConfig();
  ...
  const createWaypointIcon = (label, color) => {
    return L.divIcon({
      ...
      html: `<div style="width: 24px; height: 24px; background: ${color}; border: 2px solid #fff; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 11px; font-weight: 700; color: #000; box-shadow: 0 2px 6px rgba(0,0,0,0.4);">${label}</div>`,
      iconSize: [24, 24],
      iconAnchor: [12, 12]
    });
  };
  ...
  const createTargetIcon = (label, isPrimary) => {
    return L.divIcon({
      ...
      html: `<div style="width: ${isPrimary ? 28 : 22}px; height: ${isPrimary ? 28 : 22}px; background: ${isPrimary ? '#ef4444' : '#f97316'}; border: 2px solid #fff; border-radius: 4px; transform: rotate(45deg); display: flex; align-items: center; justify-content: center; box-shadow: 0 2px 6px rgba(0,0,0,0.4);"><span style="transform: rotate(-45deg); font-size: 9px; font-weight: 700; color: #fff;">${label}</span></div>`,
  ...
  const createDefenseIcon = (assetName) => {
    return L.divIcon({
      ...
      html: `<div style="padding: 4px 8px; background: #f97316; border: 2px solid #fff; border-radius: 4px; font-size: 9px; font-weight: 700; color: #000; box-shadow: 0 2px 6px rgba(0,0,0,0.4); white-space: nowrap;">${assetName || 'ASSET'}</div>`,
      iconSize: [80, 24],
      iconAnchor: [40, 12]
    });
  };
    ...
    >
      ...
      >
        <div className="flex items-center gap-2">
          <MapPin size={14} color={zoneStyle.color} />
          <span className="text-[0.7rem] font-semibold" style={{ color: zoneStyle.color }}>
            {zoneStyle.label.toUpperCase()}
          </span>
          <span className="text-gray-500 text-[0.55rem]">
            {getStatusText()} • ~{metric.value.toFixed(0)} {metric.unit}
          </span>
        </div>
