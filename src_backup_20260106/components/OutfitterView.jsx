...
import { Ship, X, Target, Settings, ChevronLeft, Plus, Minus, Move, Grid3X3, Maximize2, ArrowRight } from 'lucide-react';
...
const OutfitterView = ({ onBackToShipyard }) => {
  ...
  const [draggingFromMount, setDraggingFromMount] = React.useState(null);
            ...
            <div className="bg-darker rounded-xl p-8 border border-lime-brand/20 relative">
              ...
              <div className="relative w-full h-[650px] bg-darkest rounded-2xl border border-lime-brand/10 overflow-hidden mb-8">
                ...
                {vesselMountPoints[selectedHull.name] && Object.entries(vesselMountPoints[selectedHull.name]).map(([mountName, mount]) => {
                  ...
                  const isMovingSource = movingFromMount === mountName;
                  const isValidMoveTarget = movingFromMount && movingFromMount !== mountName;
                  const isDragging = draggingFromMount === mountName;
                  const isDragTarget = draggingFromMount && draggingFromMount !== mountName;

                  const handleMountClick = () => {
                    if (movingFromMount) {
                      if (movingFromMount === mountName) {
                        // Clicking source mount cancels move
                        setMovingFromMount(null);
                      } else {
                        // Move capability to this mount
                  ...
                  const handleDragOver = (e) => {
                    if (draggingFromMount && draggingFromMount !== mountName) {
                      e.preventDefault();
                  ...
                  const handleDrop = (e) => {
                    ...
                    if (fromMount && fromMount !== mountName) {
                      moveCapability(fromMount, mountName);
                    }
                    setDraggingFromMount(null);
                  };
                  ...
                  return (
                    <React.Fragment key={mountName}>
                      <div
                        ...
                        onDrop={handleDrop}
                        className={`absolute -translate-x-1/2 -translate-y-1/2 w-[50px] h-[50px] rounded-full flex items-center justify-center text-[1.4rem] font-bold transition-all duration-200 z-10
                          ${isEquipped ? 'cursor-grab active:cursor-grabbing' : 'cursor-pointer'}
                          ${isDragging ? 'opacity-50 scale-90' : ''}
                          ${isDragTarget ? 'bg-purple-400/50 text-purple-200 border-purple-400 scale-110' :
                            isMovingSource ? 'bg-blue-400 text-black animate-pulse' :
                            isValidMoveTarget ? 'bg-blue-400/40 text-blue-300 border-blue-400 hover:bg-blue-400/60' :
                            isEquipped ? 'bg-green-400 text-black' :
                            isSelected ? 'bg-lime-brand text-black' :
                            'bg-lime-brand/30 text-lime-brand'
                          }
                          ${isDragTarget ? 'border-2 border-dashed border-purple-400' :
                            isMovingSource ? 'border-[3px] border-blue-400' :
                            isValidMoveTarget ? 'border-2 border-dashed border-blue-400' :
                            isSelected ? 'border-[3px] border-lime-brand' :
                            'border-2 border-lime-brand/50'
                          }`}
                        style={{
                          left: `${mount.x}%`,
                          top: `${mount.y}%`,
                          boxShadow: isDragTarget ? '0 0 0 4px rgba(168, 85, 247, 0.4), 0 0 20px rgba(168, 85, 247, 0.3)' :
                            isMovingSource ? '0 0 0 4px rgba(96, 165, 250, 0.5), 0 0 20px rgba(96, 165, 250, 0.3)' :
                            isValidMoveTarget ? '0 0 0 4px rgba(96, 165, 250, 0.2)' :
                            isSelected ? '0 0 0 4px rgba(203, 253, 0, 0.3)' :
                            isEquipped ? '0 0 0 2px rgba(74, 222, 128, 0.5)' : 'none'
                        }}
                        title={isEquipped ? `Drag to move: ${isEquipped.name}` : mountName}
                      >
                        {isDragTarget ? <ArrowRight size={24} /> :
                         isMovingSource ? <Move size={24} /> :
                         isValidMoveTarget ? <ArrowRight size={24} /> :
                         isEquipped && EquippedIcon ? <EquippedIcon size={24} /> :
                         (isEquipped ? '✓' : '+')}
                      </div>

                      {/* Mount point label */}
                      <div
                        className="absolute -translate-x-1/2 text-xs text-lime-brand font-semibold text-center pointer-events-none bg-black/70 py-1 px-2 rounded-md whitespace-nowrap z-[5]"
                        style={{
                          left: `${mount.x}%`,
                          top: `${mount.y + 8}%`,
                ...
                {availableSlots.map((slot) => {
                  ...
                  const EquippedIcon = isEquipped?.icon;
                  const isMovingSource = movingFromMount === slot.id;
                  const isValidMoveTarget = movingFromMount && movingFromMount !== slot.id;
                  const isDragging = draggingFromMount === slot.id;
                  const isDragTarget = draggingFromMount && draggingFromMount !== slot.id;
                  ...
                  const handleCapabilityDragStart = (e) => {
                    if (!isEquipped) {
                      // If not equipped, this is a slot repositioning drag
                      e.dataTransfer.setData('slotId', slot.id);
                      return;
                    }
                    // Equipped capability drag
                    setDraggingFromMount(slot.id);
                    e.dataTransfer.effectAllowed = 'move';
                    e.dataTransfer.setData('text/plain', slot.id);
                  };

                  const handleCapabilityDragEnd = (e) => {
                    if (draggingFromMount) {
                      setDraggingFromMount(null);
                    } else if (!isEquipped) {
                      // Slot repositioning
                  ...
                  const handleDragOver = (e) => {
                    if (draggingFromMount && draggingFromMount !== slot.id) {
                      e.preventDefault();
                      e.dataTransfer.dropEffect = 'move';
                    }
                  };
                  ...
                  const handleDrop = (e) => {
                    e.preventDefault();
                    const fromMount = e.dataTransfer.getData('text/plain');
                    if (fromMount && fromMount !== slot.id) {
                      moveCapability(fromMount, slot.id);
                    }
                    setDraggingFromMount(null);
                  };
                  ...
                  return (
                    <React.Fragment key={slot.id}>
                      <div
                        ...
                        draggable
                        onDragStart={handleCapabilityDragStart}
                        onDragEnd={handleCapabilityDragEnd}
                        onDragOver={handleDragOver}
                        onDrop={handleDrop}
                        className={`absolute -translate-x-1/2 -translate-y-1/2 w-[50px] h-[50px] rounded-full flex items-center justify-center text-[1.4rem] font-bold transition-all duration-200 z-10
                          ${isEquipped ? 'cursor-grab active:cursor-grabbing' : 'cursor-move'}
                          ${isDragging ? 'opacity-50 scale-90' : ''}
                          ${isDragTarget ? 'bg-purple-400/50 text-purple-200 border-purple-400 scale-110' :
                            isMovingSource ? 'bg-blue-400 text-black animate-pulse' :
                            isValidMoveTarget ? 'bg-blue-400/40 text-blue-300 border-blue-400 hover:bg-blue-400/60' :
                          ...
                          ${isDragTarget ? 'border-2 border-dashed border-purple-400' :
                            isMovingSource ? 'border-[3px] border-blue-400' :
                            isValidMoveTarget ? 'border-2 border-dashed border-blue-400' :
                            slot.isCustom ? 'border-2 border-dashed border-lime-brand/80' :
                            'border-2 border-lime-brand/50'
                          }`}
                        style={{
                          left: `${position.x}%`,
                        ...
                        title={isEquipped ? `Drag to move: ${isEquipped.name}` : `${slot.name} (Drag to reposition)`}
                      >
                        {isDragTarget ? <ArrowRight size={24} /> :
                         isMovingSource ? <Move size={24} /> :
                         isValidMoveTarget ? <ArrowRight size={24} /> :
                      ...
                      {/* Custom slot label */}
                      <div
                        className="absolute -translate-x-1/2 text-xs text-lime-brand font-semibold text-center pointer-events-none bg-black/70 py-1 px-2 rounded-md whitespace-nowrap z-[5] border border-dashed border-lime-brand/50"
                        style={{
                          left: `${position.x}%`,
                          top: `${position.y + 8}%`,
                          textShadow: '0 0 4px rgba(0,0,0,0.8)'
