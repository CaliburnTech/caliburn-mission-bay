...
import { Play, Pause, RotateCcw, Crosshair, ChevronLeft, Settings, ArrowLeftRight } from 'lucide-react';
...
const T_COMPLETE  = 86;   // Alpha back rearmed; LCS has not moved once
...
// Alpha: forward until expended, transits to the ROS, rearm, returns forward
const getAlphaPos = (tick) => {
  if (tick < T_EXPENDED) return M48_A_POS;
  const half = (T_EXPENDED + T_COMPLETE) / 2;
  if (tick < half)       return lerp2(M48_A_POS, ROS_POS, (tick - T_EXPENDED) / (half - T_EXPENDED));
  if (tick < T_COMPLETE) return lerp2(ROS_POS, M48_B_POS, (tick - half) / (T_COMPLETE - half));
              ...
              {currentTick >= T_ONSTATION && (
                ...
                >
                  <Tooltip direction="bottom" offset={[0, 8]}>
                    <span style={{ fontSize: 10, fontWeight: 700, color: '#94a3b8' }}>Remote Operating Site — Rearm</span>
                  </Tooltip>
                </CircleMarker>
              )}
