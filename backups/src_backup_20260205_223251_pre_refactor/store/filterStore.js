...
const useFilterStore = create((set, get) => ({
  ...
  getFilteredItems: (items) => {
    ...
    // Apply security level filters
    if (state.selectedSecurityFilters.length > 0) {
      filtered = filtered.filter(item => {
        // Item must have at least one of the selected security levels
        if (!item.securityLevel || !Array.isArray(item.securityLevel)) {
          return false;
        }
        return state.selectedSecurityFilters.some(filter =>
          item.securityLevel.includes(filter)
        );
      });
    }
